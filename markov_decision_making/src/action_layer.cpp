...
actionMetadataCallback (const FactoredSymbolMetadataConstPtr& msg)
{
  ...
  if (msg->symbol_metadata.size() < (size_t) mdm_agent_index_) 
  {
  ...
  uint32_t expected_values = msg->symbol_metadata[mdm_agent_index_].number_of_symbols;
  ...
  action_sizes_.clear();
  for (size_t i = 0; i < msg->symbol_metadata.size(); i++) 
  {
    action_sizes_.push_back (msg->symbol_metadata[i].number_of_symbols);
    if(! using_named_actions_ && i == mdm_agent_index_)
    {
      for (size_t j = 0; j < msg->symbol_metadata[j].symbol_names.size(); j++)
      {
        local_action_names_.push_back(msg->symbol_metadata[i].symbol_names[j]);
  ...
  if(using_named_actions_)
  {
    for(size_t i = 0; i < msg->symbol_metadata[mdm_agent_index_].symbol_names.size(); i++) 
    {
      string name = msg->symbol_metadata[mdm_agent_index_].symbol_names[i];
