...
namespace predicate_manager
{
  ...
  class PredicateManager
  {
    ...
    private:
       ...
       */
      void consumeUpdate(const PredicateUpdateConstPtr& msg);
      /**
       * Publishes this PM's Predicate Map to the "/predicate_map" topic.
       */
      ...
      /**
       * Binds a Predicate to its dependencies. This allows the Predicate to get the values
       * of its dependencies from this PM.
      ...
      ros::Publisher predicate_update_pub_; ///Publisher for this PM's predicate updates.
      ros::Subscriber predicate_maps_sub_; ///Subscriber to the predicate maps of other PMs.
      ros::Subscriber predicate_update_sub_; ///Subscriber to the predicate updates of other PMs.
      
//       ros::Publisher event_map_pub_;
//       ros::Publisher event_update_pub_;
      ...
      std::vector<uint32_t> counters_; ///A vector of integer tokens that act as a "synchronization key" for predicate updates.
      NameIDObserver pred_name_observer_; ///The observer set for each Predicate, accessible through its NameID.
      NrIDObserver pred_nr_observer_; ///The observer set for each Predicate, accessible through its NrID.
  };
}
